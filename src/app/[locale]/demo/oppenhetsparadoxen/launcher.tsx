...
interface Props {
  scenarios: Scenario<OppenhetsparadoxenInput>[];
}

export function OppenhetsparadoxenLauncher({ scenarios }: Props) {
  ...
  const submit = React.useCallback(
    async (s: Scenario<OppenhetsparadoxenInput>) => {
      setPendingSlug(s.slug);
      setError(null);
      try {
        const res = await fetch("/api/run/oppenhetsparadoxen", {
          method: "POST",
        ...
        router.push(`/demo/oppenhetsparadoxen/runs/${payload.runId}`);
      } catch (err) {
        setPendingSlug(null);
            ...
            return (
              <Card key={s.slug} className="flex flex-col">
                ...
                <CardContent className="flex-1 space-y-2 text-xs text-muted-foreground">
                  <div>
                    {s.input.originalText.length} {t("charsOriginal")}
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {s.input.audiences.map((a) => (

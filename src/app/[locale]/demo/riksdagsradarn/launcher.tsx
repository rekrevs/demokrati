...
import type { RiksdagsradarnInput } from "@/lib/demos/riksdagsradarn/schemas";
import type { Scenario } from "@/lib/demos/module";

interface Props {
  scenarios: Scenario<RiksdagsradarnInput>[];
}

export function RiksdagsradarnLauncher({ scenarios }: Props) {
  ...
  const submit = React.useCallback(
    async (scenario: Scenario<RiksdagsradarnInput>) => {
      setPendingSlug(scenario.slug);
      setError(null);
      try {
        const res = await fetch("/api/run/riksdagsradarn", {
          method: "POST",
        ...
        router.push(`/demo/riksdagsradarn/runs/${payload.runId}`);
      } catch (err) {
        setPendingSlug(null);
